...
> `Spring Boot` `Spring AI` `RAG` `PostgreSQL pgvector` `Redis` `AWS ECS` `OpenAI` `Docker` `JWT`

A GenAI support assistant using **Retrieval-Augmented Generation** that deflected **40% of routine support tickets**.

| Metric | Result |
|---|---|
| 🎯 Ticket Deflection | 40% of routine tickets automated |
| ⚡ Latency Reduction | 45% faster via pgvector + Redis caching |
| ✅ Test Coverage | 90%+ with JUnit & Mockito |
| 🚢 Deployment | Dockerized on AWS ECS behind API Gateway |

---

### 🛒 Cloud-Native E-Commerce Microservices Platform
> `Java 17` `Spring Boot` `Spring Cloud Gateway` `AWS EKS` `Kafka` `React` `OAuth 2.0` `Stripe` `Terraform`

End-to-end e-commerce platform — catalog, cart, order & payment microservices.

| Metric | Result |
|---|---|
| 📦 Events Processed | 100K+ Kafka events/day |
| ☁️ Infrastructure | AWS EKS provisioned via Terraform |
| 🔐 Security | OAuth 2.0 + Stripe payment integration |
| 🏗️ Architecture | 4 independent microservices with Spring Cloud Gateway |

---

### 📊 Real-Time Attendance Monitoring Platform
> `Spring Boot` `React.js` `MongoDB` `Twilio` `RFID` `Biometric` `QR Code`

Secure, multi-modal attendance platform for large institutions.

| Metric | Result |
|---|---|
| 👥 Scale | 2,000+ students tracked |
| 📡 Tracking Modes | RFID, Biometric & QR-code |
| 📱 Alerts | Real-time SMS via Twilio |
| 🔒 Access Control | Role-based dashboards (Admin / Faculty / Student) |
